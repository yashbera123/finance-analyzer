"""
Profile Endpoint
================
POST /profile/{file_id} — full pipeline: parse → categorize → analyze → profile.
Returns financial personality, scores, and actionable recommendations.
"""

from pathlib import Path

from fastapi import APIRouter, HTTPException, status

from app.schemas.profile import ProfileResponse
from app.services.analyzer import run_analysis
from app.services.categorizer import categorize_transactions
from app.services.parser import parse_transactions, validate_parse_result
from app.services.profiler import generate_profile
from app.utils.file_helpers import UPLOAD_DIR

router = APIRouter()


def _find_uploaded_file(file_id: str) -> Path:
    """Locate an uploaded file by its ID."""
    upload_dir = Path(UPLOAD_DIR)
    if not upload_dir.exists():
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Upload directory not found.",
        )
    matches = list(upload_dir.glob(f"{file_id}.*"))
    if not matches:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"No uploaded file found for file_id: {file_id}",
        )
    return matches[0]


@router.post(
    "/{file_id}",
    response_model=ProfileResponse,
    summary="Generate financial behavior profile",
    description="Full pipeline: parse → categorize → analyze → profile. "
                "Returns spending personality, risk level, scores, "
                "and actionable recommendations.",
)
async def profile_file(file_id: str):
    """
    Generate a complete financial behavior profile.

    Steps:
    1. Parse the uploaded file
    2. Categorize transactions
    3. Run behavioral analysis
    4. Generate profile + recommendations
    """
    file_path = _find_uploaded_file(file_id)

    # Parse
    try:
        parse_result = parse_transactions(
            file_path=str(file_path),
            file_id=file_id,
            original_filename=file_path.name,
        )
        validate_parse_result(parse_result)
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"Failed to parse file: {str(e)}",
        )

    # Categorize
    cat_result = categorize_transactions(parse_result)

    # Analyze
    analysis = run_analysis(file_id=file_id, transactions=cat_result.transactions)

    # Profile
    try:
        result = generate_profile(file_id=file_id, analysis=analysis)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Profile generation failed: {str(e)}",
        )

    return ProfileResponse(
        status="success",
        message="Financial profile generated",
        data=result,
    )
